...
async function benchmarkTokenList() {
  ...
  for (let i = 0; i < Math.min(iterations, 5); i++) {
    // Fewer iterations for token list
    const start = Date.now();
  ...
  const testConcurrentRequests = async (network, label) => {
    ...
    for (let batch = 0; batch < Math.floor(iterations / concurrent); batch++) {
      ...
      for (let i = 0; i < concurrent; i++) {
        const promise = axios
          .post(`${GATEWAY_URL}/chains/solana/balances`, {
...
async function runBenchmarks() {
  ...
  // Error summary
  const totalHeliusErrors =
    (results.balanceFetching?.errors?.helius || 0) + (results.concurrent?.errors?.helius?.length || 0);
  const totalStandardErrors =
    (results.balanceFetching?.errors?.standard || 0) + (results.concurrent?.errors?.standard?.length || 0);
