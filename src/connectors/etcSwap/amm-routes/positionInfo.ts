...
export const positionInfoRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get position information for a ETCSwap V2 pool',
    ...
    async (request) => {
      try {
        ...
        const baseTokenObj = etcSwap.getTokenByAddress(token0);
        const quoteTokenObj = etcSwap.getTokenByAddress(token1);
