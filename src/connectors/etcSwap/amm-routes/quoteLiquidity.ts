...
import { IETCSwapV2PairABI, getETCSwapV2RouterAddress } from '../etcSwap.contracts';
...
export async function getETCSwapAmmLiquidityQuote(
  ...
  if (existingPool) {
    ...
    const pairContract = new Contract(poolAddressToUse, IETCSwapV2PairABI.abi, ethereum.provider);
...
export const quoteLiquidityRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get liquidity quote for a ETCSwap V2 pool',
