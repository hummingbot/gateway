import { Position, tickToPrice, computePoolAddress } from '@_etcswap/v3-sdk';
...
import { POSITION_MANAGER_ABI, getETCSwapV3NftManagerAddress, getETCSwapV3FactoryAddress } from '../etcSwap.contracts';
...
export const positionInfoRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get position information for a ETCSwap V3 position',
    ...
    async (request) => {
      try {
        const {
          network,
          positionAddress, // This will be the NFT token ID for ETCSwap V3
        ...
        // In ETCSwap, typically the token with the lower address is token0
        const isBaseToken0 =
          token0.symbol === 'WETC' ||
        ...
        const poolAddress = computePoolAddress({
          factoryAddress: getETCSwapV3FactoryAddress(networkToUse),
          tokenA: token0,
          tokenB: token1,
          fee,
        });
