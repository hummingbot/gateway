// import {  } from '@_etcswap/router-sdk';
import { Trade as RouterTrade, Protocol } from '@_etcswap/router-sdk';
import { TradeType, Percent, Currency, CurrencyAmount, Token } from '@_etcswap/sdk-core';
import { SwapRouter, SwapOptions, UNIVERSAL_ROUTER_ADDRESS, UniswapTrade } from '@_etcswap/universal-router-sdk';
import { Pair as V2Pair, Route as V2Route, Trade as V2Trade, computePairAddress } from '@_etcswap/v2-sdk';
import IETCSwapV3Pool from '@_etcswap/v3-core/artifacts/contracts/interfaces/IUniswapV3Pool.sol/IUniswapV3Pool.json';
import {
  Pool as V3Pool,
  Route as V3Route,
  Trade as V3Trade,
...
import { IETCSwapV2PairABI, getETCSwapV3FactoryAddress, getETCSwapV2FactoryAddress } from './etcSwap.contracts';
...
export class UniversalRouterService {
  ...
  async getQuote(
    ...
    if (protocols.includes(Protocol.V3)) {
      try {
        ...
        if (v3Trade) {
          ...
          } as unknown as V3Trade<Currency, Currency, TradeType>);
        }
      } catch (error) {
    ...
    if (protocols.includes(Protocol.V2)) {
      try {
        ...
        if (v2Trade) {
          ...
          } as unknown as V2Trade<Currency, Currency, TradeType>);
        }
      } catch (error) {
    ...
    // Use the underlying trade(s) for UniswapTrade constructor
    // const underlyingTrade = bestTrade.v3Routes.length > 0 ? bestTrade.v3Routes[0] : bestTrade.v2Routes[0];
    const { calldata, value } = SwapRouter.swapCallParameters(new UniswapTrade(bestRoute, swapOptions));
    ...
    for (const fee of V3_FEE_TIERS) {
      try {
        ...
        const poolAddress = computePoolAddress({
          factoryAddress: getETCSwapV3FactoryAddress(this.network),
          tokenA: tokenIn,
          tokenB: tokenOut,
          fee,
        });
        ...
        // Get pool contract
        const poolContract = new Contract(poolAddress, IETCSwapV3Pool.abi, this.provider);
  ...
  private async findV2Route(
    ...
    try {
      ...
      const pairAddress = computePairAddress({
        factoryAddress: getETCSwapV2FactoryAddress(this.network),
        tokenA: tokenIn,
        tokenB: tokenOut,
        initCodeHashManualOverride: '0xb5e58237f3a44220ffc3dfb989e53735df8fcd9df82c94b13105be8380344e52', // ETCSwap V2 init code hash
      });

      const pairContract = new Contract(pairAddress, IETCSwapV2PairABI.abi, this.provider);
