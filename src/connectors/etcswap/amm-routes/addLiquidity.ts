...
async function addLiquidity(
  ...
  if (baseToken === 'ETC') {
    ...
    const wetcToken = await etcswap.getToken('WETC');
    if (!wetcToken) {
...
export const addLiquidityRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{
    Body: Static<typeof ETCswapAmmAddLiquidityRequest>;
    Reply: AddLiquidityResponseType;
  }>(
    '/add-liquidity',
    {
      schema: {
        description: 'Add liquidity to an ETCswap V2 pool',
        tags: ['/connector/etcswap'],
        body: ETCswapAmmAddLiquidityRequest,
        response: {
