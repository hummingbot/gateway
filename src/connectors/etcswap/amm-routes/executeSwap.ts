...
export async function executeAmmSwap(
  ...
  // Find pool address
  const poolAddress = await etcswap.findDefaultPool(baseToken, quoteToken, 'amm');
  if (!poolAddress) {
    throw httpErrors.notFound(`No AMM pool found for pair ${baseToken}-${quoteToken}`);
  }

  // Get quote using the shared quote function
  const { quote } = await getETCswapAmmQuote(network, poolAddress, baseToken, quoteToken, amount, side, slippagePct);

  // Check if this is a hardware wallet
  const isHardwareWallet = await ethereum.isHardwareWallet(walletAddress);

  // Get Router02 contract address
  const routerAddress = getETCswapV2RouterAddress(network);

  logger.info(`Executing swap using ETCswap V2 Router:`);
...
export const executeSwapRoute: FastifyPluginAsync = async (fastify) => {
  fastify.post<{
    ...
    {
      schema: {
        description: 'Execute a swap on ETCswap V2 AMM',
        tags: ['/connector/etcswap'],
        body: ETCswapAmmExecuteSwapRequest,
    ...
    async (request) => {
      try {
        ...
        const {
          ...
          side = 'SELL',
          slippagePct,
        } = request.body as typeof ETCswapAmmExecuteSwapRequest._type;
