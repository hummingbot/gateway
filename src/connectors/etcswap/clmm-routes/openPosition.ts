import { nearestUsableTick } from '@etcswapv3/sdk';
import { Contract } from '@ethersproject/contracts';
import { Percent } from '@uniswap/sdk-core';
import { Position, NonfungiblePositionManager, MintOptions } from '@uniswap/v3-sdk';
...
import { formatTokenAmount, getETCswapPoolInfo, toUniswapPool } from '../etcswap.utils';
...
export async function openPosition(
  ...
  // Get the V3 pool (ETCswap Pool)
  const etcswapPool = await etcswap.getV3Pool(baseTokenObj, quoteTokenObj, undefined, poolAddress);
  if (!etcswapPool) {
    throw httpErrors.notFound(`Pool not found for ${baseTokenObj.symbol}-${quoteTokenObj.symbol}`);
  }

  // Convert to Uniswap Pool for position management (needed for NonfungiblePositionManager)
  ...
  const token0 = etcswapPool.token0;
  const token1 = etcswapPool.token1;
  ...
  // Create the position using Uniswap's Position class with the converted pool
  ...
  if (!JSBI.equal(amount0Raw, JSBI.BigInt(0))) {
  ...
  if (!JSBI.equal(amount1Raw, JSBI.BigInt(0))) {
  ...
  // For position rent, we're using 0 since Ethereum Classic doesn't have rent like Solana
...
export const openPositionRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{
    ...
    {
      schema: {
        description: 'Open a new liquidity position in an ETCswap V3 pool',
        ...
        body: {
          ...
          properties: {
            ...
            lowerPrice: { type: 'number', examples: [50] },
            upperPrice: { type: 'number', examples: [200] },
            poolAddress: { type: 'string', examples: ['0x0000000000000000000000000000000000000000'] },
