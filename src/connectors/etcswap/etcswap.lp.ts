...
import { ETCSwapLPHelper, FACTORY, POOL_INIT } from './etcswap.lp.helper';
import { AddPosReturn } from '../uniswap/uniswap.lp.interfaces';
...
export class ETCSwapLP extends ETCSwapLPHelper implements UniswapLPish {
  private static _instances: { [name: string]: ETCSwapLP };
  ...
  public static getInstance(chain: string, network: string): ETCSwapLP {
    if (ETCSwapLP._instances === undefined) {
      ETCSwapLP._instances = {};
    }
    if (!(chain + network in ETCSwapLP._instances)) {
      ETCSwapLP._instances[chain + network] = new ETCSwapLP(
        chain,
        network,
      );
    }

    return ETCSwapLP._instances[chain + network];
  }
