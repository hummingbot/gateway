...
import { getUniversalRouterAddress } from '../etcswap.contracts';
...
async function executeQuote(walletAddress: string, network: string, quoteId: string): Promise<SwapExecuteResponseType> {
  ...
  logger.info(
    `Executing ETCswap quote ${quoteId} for ${amount} ${inputToken.symbol} -> ${outputToken.symbol}${isHardwareWallet ? ' with hardware wallet' : ''}`,
  );

  // Get the ETCswap Universal Router address
  const universalRouterAddress = getUniversalRouterAddress(network);

  // Check and approve allowance if needed - Universal Router uses Permit2
  if (inputToken.address !== ethereum.nativeTokenSymbol) {
    ...
    if (BigNumber.from(tokenToPermit2Allowance).lt(requiredAllowance)) {
      ...
      throw httpErrors.badRequest(
        `Insufficient ${inputToken.symbol} allowance to Permit2. ` +
          ...
          `Please approve ${inputToken.symbol} using spender: "etcswap/router"`,
      );
    }

    // Step 2: Check Permit2's allowance to Universal Router
    logger.info(`Checking Permit2 allowance to ETCswap Universal Router (${universalRouterAddress})`);
    ...
    if (isExpired || BigNumber.from(permit2Amount).lt(requiredAllowance)) {
      ...
      if (isExpired) {
        const expirationDate = new Date(expiration * 1000);
        throw httpErrors.badRequest(
          `Permit2 allowance for ${inputToken.symbol} to ETCswap Universal Router has expired. ` +
            `Expired at: ${expirationDate.toISOString()}. ` +
            `Please approve ${inputToken.symbol} again using spender: "etcswap/router"`,
        );
      } else {
        throw httpErrors.badRequest(
          `Insufficient Permit2 allowance for ${inputToken.symbol} to ETCswap Universal Router. ` +
  ...
  try {
    if (isHardwareWallet) {
      ...
      // Get gas options with increased gas limit for Universal Router
    ...
    if (error.error && error.error.data) {
      ...
      errorSelector = errorData.substring(0, 10);
    ...
    if (errorSelector === '0xd81b2f2e') {
      ...
      throw httpErrors.badRequest(
        `ETCswap Universal Router error: Permit2 allowance has expired for ${inputToken.symbol}. ` +
          `Please re-approve the token using spender: "etcswap/router" to set a new expiration.`,
      );
  ...
  // Use the handleExecuteQuoteTransactionConfirmation helper
