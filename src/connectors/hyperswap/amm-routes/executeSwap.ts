...
import { getHyperswapV2RouterAddress, IHyperswapV2Router02ABI } from '../hyperswap.contracts';
import { formatTokenAmount } from '../hyperswap.utils';
import { HyperswapAmmExecuteSwapRequest } from '../schemas';

import { getHyperswapAmmQuote } from './quoteSwap';
...
export async function executeAmmSwap(
  ...
  const hyperswap = await Hyperswap.getInstance(network);

  // Find pool address
  const poolAddress = await hyperswap.findDefaultPool(baseToken, quoteToken, 'amm');
  if (!poolAddress) {
    throw httpErrors.notFound(`No AMM pool found for pair ${baseToken}-${quoteToken}`);
  }

  // Get quote using the shared quote function
  const { quote } = await getHyperswapAmmQuote(network, poolAddress, baseToken, quoteToken, amount, side, slippagePct);

  // Check if this is a hardware wallet
  const isHardwareWallet = await ethereum.isHardwareWallet(walletAddress);

  // Get Router02 contract address
  const routerAddress = getHyperswapV2RouterAddress(network);
  ...
  if (currentAllowance.lt(amountNeeded)) {
    ...
    throw httpErrors.badRequest(
      `Insufficient allowance for ${quote.inputToken.symbol}. Please approve at least ${formatTokenAmount(amountNeeded, quote.inputToken.decimals)} ${quote.inputToken.symbol} for the Hyperswap router (${routerAddress})`,
    );
  }
  ...
  try {
    if (isHardwareWallet) {
      ...
      const iface = new utils.Interface(IHyperswapV2Router02ABI.abi);
      let data;
      ...
      const routerContract = new Contract(routerAddress, IHyperswapV2Router02ABI.abi, wallet);
...
export const executeSwapRoute: FastifyPluginAsync = async (fastify) => {
  fastify.post<{
    ...
    {
      schema: {
        description: 'Execute a swap on Hyperswap V2 AMM using Router02',
        tags: ['/connector/hyperswap'],
        body: HyperswapAmmExecuteSwapRequest,
