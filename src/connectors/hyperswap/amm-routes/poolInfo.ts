...
import { HyperswapAmmGetPoolInfoRequest } from '../schemas';
...
export const poolInfoRoute: FastifyPluginAsync = async (fastify) => {
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get AMM pool information from Hyperswap V2',
        tags: ['/connector/hyperswap'],
        querystring: HyperswapAmmGetPoolInfoRequest,
        response: {
          200: PoolInfoSchema,
        },
      },
    ...
    async (request): Promise<PoolInfo> => {
      try {
        ...
        // For Hyperswap, we need to get the pair contract to extract token addresses
        ...
        const token0 = await hyperswap.getToken(token0Address);
        const token1 = await hyperswap.getToken(token1Address);
        ...
        // Get V2 pair data
        const v2Pair = await hyperswap.getV2Pool(token0, token1, poolAddress);
        ...
        return {
          ...
          feePct: 0.3, // Hyperswap V2 fee is fixed at 0.3%
          price: price,
