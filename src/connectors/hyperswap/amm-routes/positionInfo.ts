...
export async function checkLPAllowance(
  ...
  if (currentLpAllowance.lt(requiredAmount)) {
    throw new Error(
      `Insufficient LP token allowance. Please approve at least ${formatTokenAmount(requiredAmount.toString(), 18)} LP tokens (${poolAddress}) for the Hyperswap router (${routerAddress})`,
    );
  }
}
...
export const positionInfoRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get position information for a Hyperswap V2 pool',
    ...
    async (request) => {
      try {
        ...
        const baseTokenObj = await hyperswap.getToken(token0);
        const quoteTokenObj = await hyperswap.getToken(token1);
