...
import { IHyperswapV2PairABI, getHyperswapV2RouterAddress } from '../hyperswap.contracts';
import { formatTokenAmount, getHyperswapPoolInfo } from '../hyperswap.utils';

export async function getHyperswapAmmLiquidityQuote(
  network: string,
  poolAddress?: string,
  baseToken?: string,
  quoteToken?: string,
  ...
  if (!poolAddressToUse) {
    poolAddressToUse = await hyperswap.findDefaultPool(baseToken, quoteToken, 'amm');
  ...
  if (existingPool) {
    ...
    const pairContract = new Contract(poolAddressToUse, IHyperswapV2PairABI.abi, ethereum.provider);
...
export const quoteLiquidityRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get liquidity quote for a Hyperswap V2 pool',
    ...
    async (request) => {
      try {
        ...
        const poolInfo = await getHyperswapPoolInfo(poolAddress, network, 'amm');
        if (!poolInfo) {
