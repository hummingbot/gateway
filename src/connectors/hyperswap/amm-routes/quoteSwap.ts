...
async function quoteAmmSwap(
  ...
  try {
    // Get the V2 pair
    const pair = await hyperswap.getV2Pool(baseToken, quoteToken, poolAddress);
    if (!pair) {
    ...
    // Check for insufficient reserves error from Hyperswap SDK
...
export async function getHyperswapAmmQuote(
  network: string,
  poolAddress: string,
  baseToken: string,
  quoteToken: string,
  ...
  // Resolve tokens
  const baseTokenObj = await hyperswap.getToken(baseToken);
  const quoteTokenObj = await hyperswap.getToken(quoteToken);
...
async function formatSwapQuote(
  ...
  try {
    ...
    const { quote, ethereum } = await getHyperswapAmmQuote(
      network,
      poolAddress,
      baseToken,
      quoteToken,
...
export const quoteSwapRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get swap quote for Hyperswap V2 AMM',
    ...
    async (request) => {
      try {
        ...
        if (poolAddressToUse) {
          ...
          const poolInfo = await getHyperswapPoolInfo(poolAddressToUse, networkToUse, 'amm');
          if (!poolInfo) {
          ...
          if (baseToken === poolInfo.baseTokenAddress) {
            ...
            const resolvedToken = await hyperswap.getToken(baseToken);
          ...
          poolAddressToUse = await hyperswap.findDefaultPool(baseTokenToUse, quoteTokenToUse, 'amm');
