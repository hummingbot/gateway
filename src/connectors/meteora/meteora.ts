...
export class Meteora {
  ...
  /**
   * Subscribe to real-time DLMM pool updates via WebSocket
  ...
  public async subscribeToPoolUpdates(
    ...
    const heliusService = this.solana.getHeliusService();
    ...
    const subscriptionId = await heliusService.subscribeToAccount(
      poolAddress,
      async (_accountInfo, context) => {
        try {
          // Refetch pool state to get latest data
          const dlmmPool = await this.getDlmmPool(poolAddress);
          await dlmmPool.refetchStates();
          ...
          const reserveX = Number(convertDecimals(dlmmPool.lbPair.reserveX, dlmmPool.tokenX.mint.decimals));
          const reserveY = Number(convertDecimals(dlmmPool.lbPair.reserveY, dlmmPool.tokenY.mint.decimals));

          // Parse fee percentage - use baseFactor from parameters
          const baseFactor = dlmmPool.lbPair.parameters.baseFactor;
          const feePct = baseFactor / 10000; // Convert basis points to percentage

          // Adjust price for decimal difference
          const decimalDiff = dlmmPool.tokenX.mint.decimals - dlmmPool.tokenY.mint.decimals;
          const adjustmentFactor = Math.pow(10, decimalDiff);
          const adjustedPrice = Number(activeBin.pricePerToken) * adjustmentFactor;
          ...
          logger.error(`Error processing pool update for ${poolAddress}: ${error.message}`);
        }
      },
      { encoding: 'base64', commitment: 'confirmed' },
    );

    logger.info(`Subscribed to pool updates for ${poolAddress} (subscription ID: ${subscriptionId})`);
   ...
   */
  public async unsubscribeFromPool(subscriptionId: number): Promise<void> {
    const heliusService = this.solana.getHeliusService();
    if (!heliusService) {
    ...
    logger.info(`Unsubscribed from pool updates (subscription ID: ${subscriptionId})`);
  }
}
