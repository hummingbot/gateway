...
import { PositionInfoSchema as AMMPositionInfoSchema } from '../../../schemas/amm-schema';
import {
...
export const positionsOwnedRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    Reply: CLMMAllPositionsOwnedResponseType;
  }>(
    '/positions-owned',
    {
      schema: {
        description: 'Get all CLMM positions for wallet address. Warning: Spams RPC to do so (only way for CL pools).',
    ...
    async (request) => {
      try {
        ...
        return (await positionsOwned(fastify, request.query, 'clmm')) as unknown as CLMMPositionInfo[];
      } catch (e) {
        logger.error(e);
        if (e.statusCode) {
          throw e;
