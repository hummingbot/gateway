...
export async function createPool(
  ...
  logger.info(
    `Seeding Pancakeswap V2 pool at ${seedPrice} ${quoteTokenInfo.symbol}/${baseTokenInfo.symbol} [${seedSource}]: ` +
  ...
  const factoryAddress = getPancakeswapV2FactoryAddress(network);
  const routerAddress = getPancakeswapV2RouterAddress(network);
  const factory = new Contract(factoryAddress, IPancakeswapV2FactoryABI.abi, ethereum.provider);
  ...
  if (existingPair && existingPair !== constants.AddressZero) {
    const pairContract = new Contract(existingPair, IPancakeswapV2PairABI.abi, ethereum.provider);
    ...
    if (currentAllowance.lt(erc20RawAmount)) {
      throw new Error(
        `Insufficient allowance for ${erc20TokenInfo.symbol}. Please approve at least ` +
          ...
          `for the Pancakeswap router (${routerAddress})`,
      );
    }
