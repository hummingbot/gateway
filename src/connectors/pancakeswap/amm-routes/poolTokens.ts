...
/** The token shape returned by Pancakeswap.getToken (address/decimals/symbol). */
type PancakeToken = NonNullable<Awaited<ReturnType<Pancakeswap['getToken']>>>;

/**
 * Reads a Pancakeswap V2 pair's token0/token1 and resolves them. `base` follows the pair's token0
 * orientation and `quote` its token1 — matching how pool-info / position-info report.
 */
export async function getAmmPoolTokens(
  network: string,
  poolAddress: string,
): Promise<{ base: PancakeToken; quote: PancakeToken }> {
  const pancakeswap = await Pancakeswap.getInstance(network);
  const ethereum = await Ethereum.getInstance(network);
  const pair = new Contract(poolAddress, IPancakeswapV2PairABI.abi, ethereum.provider);
  const [t0, t1] = await Promise.all([pair.token0(), pair.token1()]);
  const base = await pancakeswap.getToken(t0);
  const quote = await pancakeswap.getToken(t1);
  if (!base || !quote) {
...
export async function resolveSwapPair(
  ...
  const pancakeswap = await Pancakeswap.getInstance(network);
  const baseObj = await pancakeswap.getToken(baseToken);
