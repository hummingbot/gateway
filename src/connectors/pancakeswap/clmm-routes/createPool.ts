...
const VALID_FEE_TIERS = [100, 500, 2500, 10000];
...
export async function createPool(
  ...
  if (fee === undefined) {
    throw httpErrors.badRequest('fee tier is required (one of 100, 500, 2500, 10000)');
  }
  if (!VALID_FEE_TIERS.includes(fee)) {
    throw httpErrors.badRequest(
      `Invalid fee tier ${fee}. Must be one of 100 (0.01%), 500 (0.05%), 2500 (0.25%), 10000 (1.00%)`,
    );
  }
  ...
  logger.info(
    `Creating Pancakeswap V3 pool ${baseTokenInfo.symbol}/${quoteTokenInfo.symbol} (fee ${fee}) seeded at ` +
  ...
  const factoryAddress = getPancakeswapV3FactoryAddress(network);
  const factory = new Contract(factoryAddress, IPancakeswapV3FactoryABI, ethereum.provider);
  ...
  if (existingPool && existingPool !== constants.AddressZero) {
    const poolContract = new Contract(existingPool, IPancakeswapV3PoolSlot0ABI, ethereum.provider);
  ...
  // Create + initialize in a single idempotent call via the NonfungiblePositionManager. Pancakeswap V3 is a
  // Uniswap V3 fork, so its NFT manager exposes createAndInitializePoolIfNecessary; it deploys the pool
  // through the PoolDeployer (if needed) and initializes it at sqrtPriceX96 (if needed) — preferred over the
  // two-tx Factory.createPool + Pool.initialize path.
