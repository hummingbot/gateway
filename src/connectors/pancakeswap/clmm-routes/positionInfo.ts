  ...
  fastify.get<{
    ...
    async (request) => {
      try {
        const {
          network,
          positionAddress, // This will be the NFT token ID for Pancakeswap V3
        ...
        // In Pancakeswap, typically the token with the lower address is token0
        const isBaseToken0 =
          token0.symbol === 'WETH' ||
        ...
        const poolAddress = computePoolAddress({
          deployerAddress: getPancakeswapV3PoolDeployerAddress(networkToUse),
          tokenA: token0,
          tokenB: token1,
          fee,
        });
