...
const MasterChefStakeSchema = Type.Object({
  ...
  walletAddress: Type.String({
    description: 'The wallet address that will sign and send the staking transaction.',
...
const MasterChefStakeResponse = Type.Object({
  ...
  cakePerSecond: Type.Number({
    description: 'CAKE tokens distributed per second across the entire pool.',
  }),
    ...
    '/masterchef-stake',
    {
      schema: {
        ...
        description:
          ...
          'The response includes full position details (token addresses, price range, liquidity) and MasterChef reward metadata.',
