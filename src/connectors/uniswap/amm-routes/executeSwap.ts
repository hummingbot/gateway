    ...
    async (request) => {
      try {
        ...
        if (exactIn) {
          ...
          logger.info(`Created EXACT_INPUT trade with input amount: ${formatTokenAmount(inputAmount.quotient.toString(), inputToken.decimals)} ${inputToken.symbol}`);
        } else {
          ...
          logger.info(`Creating EXACT_OUTPUT trade with output amount: ${formatTokenAmount(outputAmount.quotient.toString(), outputToken.decimals)} ${outputToken.symbol}`);
        ...
        logger.info(`Transaction options: ${JSON.stringify({
        ...
        if (exactIn) {
          ...
          if (inputToken.symbol === 'WETH') {
            ...
            // Check if we already have sufficient allowance to avoid unnecessary approvals
            ...
            logger.info(`Current allowance: ${formatTokenAmount(currentAllowance.toString(), inputToken.decimals)} ${inputToken.symbol}`);
            logger.info(`Amount needed: ${formatTokenAmount(amountNeeded, inputToken.decimals)} ${inputToken.symbol}`);
            
            if (currentAllowance.lt(amountNeeded)) {
              logger.info(`Approving ${inputToken.symbol} for spending by router...`);
              ...
              const approvalTx = await tokenContract.approve(
                ...
                {
                  ...txOptions,
                  // Use slightly higher gas price for approval to prevent replacement
                  gasPrice: gasPrice.mul(105).div(100) // 5% higher
                }
              );
              
              logger.info(`Approval transaction submitted: ${approvalTx.hash}`);
              const approvalReceipt = await approvalTx.wait();
              logger.info(`Approval confirmed in block ${approvalReceipt.blockNumber}`);
              
              // Small delay to ensure the approval is propagated
              await new Promise(resolve => setTimeout(resolve, 2000));
              
              // Update nonce after approval transaction
              txOptions.nonce = await wallet.getTransactionCount("latest");
              logger.info(`Updated nonce to ${txOptions.nonce} after approval transaction`);
            } else {
          ...
          logger.info(`BUY direction - amountOut: ${amountOut}, amountInMax: ${amountInMax}`);
          ...
          if (inputToken.symbol === 'WETH') {
            ...
            logger.info(`Current allowance: ${formatTokenAmount(currentAllowance.toString(), inputToken.decimals)} ${inputToken.symbol}`);
            logger.info(`Amount needed: ${formatTokenAmount(amountInMax, inputToken.decimals)} ${inputToken.symbol}`);
            
            if (currentAllowance.lt(amountInMax)) {
              logger.info(`Approving ${inputToken.symbol} for spending by router...`);
        ...
        logger.info(`Swap completed - Input: ${totalInputSwapped} ${inputToken.symbol}, Output: ${totalOutputSwapped} ${outputToken.symbol}`);
        ...
        logger.info(`Gas used: ${actualGasUsed.toString()} (limit was ${gasLimit})`);
        logger.info(`Gas price: ${formatTokenAmount(actualGasPrice.toString(), 9)} Gwei`);
        ...
        if (e.code === 'TRANSACTION_REPLACED') {
          ...
          logger.info(`Transaction was replaced. Checking replacement transaction status...`);
          
          // If the replacement transaction was successful (status=1), we can use that instead
          if (receipt && receipt.status === 1) {
            logger.info(`Replacement transaction ${replacement.hash} was successful!`);
            ...
            if (replacement.data && replacement.data.startsWith('0x095ea7b3')) {
              ...
              logger.info(`Replacement was an approval transaction. Please try the swap again.`);
              
              const error = new Error(`Token approval succeeded. Please try the swap transaction again.`);
        ...
        } else if (e.message.includes('nonce too low') || e.message.includes('nonce has already been used')) {
