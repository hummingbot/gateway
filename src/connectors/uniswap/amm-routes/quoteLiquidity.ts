      ...
      try {
        ...
        if (!baseToken || !quoteToken) {
          throw fastify.httpErrors.badRequest(
            'Base token and quote token are required',
          );
        }
        ...
        if (existingPool) {
          ...
          if (baseAmountRaw && quoteAmountRaw) {
            ...
            const baseToQuoteRatio = baseReserve.isZero()
              ? BigNumber.from(0)
              : baseReserve.mul(Math.pow(10, 18)).div(quoteReserve);
            const quoteToBaseRatio = quoteReserve.isZero()
              ? BigNumber.from(0)
              : quoteReserve.mul(Math.pow(10, 18)).div(baseReserve);

            const quoteOptimal = baseAmountRaw
              .mul(quoteReserve)
              .div(baseReserve);
            ...
            if (quoteOptimal.lte(quoteAmountRaw)) {
              ...
              baseLimited = false;
              const baseOptimal = quoteAmountRaw
                .mul(baseReserve)
                .div(quoteReserve);
