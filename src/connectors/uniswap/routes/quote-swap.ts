...
export async function getUniswapQuote(
  ...
  try {
    ...
    logger.info(`Debug currencyAmount raw: ${currencyAmount.quotient.toString()}`);
  ...
  if (route.route && route.route.length > 0) {
    route.route.forEach((pool, index) => {
      if ('fee' in pool) {
        logger.info(`Route pool ${index + 1}: ${pool.token0.symbol}/${pool.token1.symbol} - Fee: ${pool.fee} (${pool.fee / 10000}%)`);
      }
    });
  }
  ...
  if (route.methodParameters) {
    ...
    if (calldataHex.length > 200 && functionSelector === '0x5ae401dc') {
      ...
      if (exactInputPos > -1) {
        ...
        const innerFunctionSelector = '0x' + calldataHex.slice(exactInputPos, exactInputPos + 8);
        ...
        if (innerFunctionSelector === '0x04e45aaf') {
          ...
          offset += 64;
          const fee = parseInt('0x' + calldataHex.slice(offset + 56, offset + 64), 16);
          offset += 64;
          ...
          const amountInEther = Number(formatTokenAmount(amountInWei.toString(), 18));
          ...
          if (exactIn) {
            ...
            logger.warn(`BUY order is using exactInputSingle instead of exactOutputSingle!`);
          }
        }
        
      } else if (exactOutputPos > -1) {
        logger.info(`Found exactOutputSingle at position ${exactOutputPos}`);
        const innerFunctionSelector = '0x' + calldataHex.slice(exactOutputPos, exactOutputPos + 8);
        logger.info(`Inner function selector: ${innerFunctionSelector}`);
      }
    }
  ...
  if (!exactIn) {
    logger.info(`BUY order debug - tradeAmount: ${tradeAmount.toExact()} ${outputToken.symbol}, route.quote: ${route.quote.toExact()} ${inputToken.symbol}`);
  }
...
export const quoteSwapRoute: FastifyPluginAsync = async (fastify, _options) => {
  ...
  fastify.get<{
    ...
    '/quote-swap',
    {
      schema: {
        description:
          'Get a swap quote using Uniswap AlphaRouter',
        tags: ['uniswap'],
        querystring: {
          type: 'object',
          properties: {
