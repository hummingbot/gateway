...
export class Xdcswap implements Uniswapish {
  ...
  private constructor(network: string) {
    const config = XdcswapConfig.config;
  ...
  public static getInstance(chain: string, network: string): Xdcswap {
    if (Xdcswap._instances === undefined) {
      Xdcswap._instances = {};
    }
    if (!(chain + network in Xdcswap._instances)) {
      Xdcswap._instances[chain + network] = new Xdcswap(network);
    }

    return Xdcswap._instances[chain + network];
  }
   ...
   * @param xdcswapRouter smart contract address
  ...
  async executeTrade(
    ...
    const contract = new Contract(xdcswapRouter, abi, wallet);
    if (!nonce) {
