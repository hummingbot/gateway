...
class AddLiquidityController extends MeteoraController {
  async addLiquidity(
    ...
    const totalXAmount = new BN(baseTokenAmount).mul(new BN(10).pow(new BN(matchingPositionInfo.tokenX.decimal)));
    const totalYAmount = new BN(quoteTokenAmount).mul(new BN(10).pow(new BN(matchingPositionInfo.tokenY.decimal)));
