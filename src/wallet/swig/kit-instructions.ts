 ...
 *
 * Kit-native connectors (Orca v4) produce kit `Instruction` objects, but the Swig
 * `@swig-wallet/classic` SDK wraps `@solana/web3.js` `TransactionInstruction`s. This
 * adapter bridges the two so a kit-built swap can be wrapped by the Swig program.
 */
