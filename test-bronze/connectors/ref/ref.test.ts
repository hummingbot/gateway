...
describe('verify Ref estimateSellTrade', () => {
  it('Should return an ExpectedTrade when available', async () => {
    ...
    const expectedTrade = await ref.estimateSellTrade(ETH, DAI, '1');
  ...
  it('Should throw an error if no pair is available', async () => {
    ...
    await expect(async () => {
      await ref.estimateSellTrade(ETH, DAI, '1');
...
describe('verify Ref estimateBuyTrade', () => {
  it('Should return an ExpectedTrade when available', async () => {
    ...
    const expectedTrade = await ref.estimateBuyTrade(ETH, DAI, '1');
    expect(expectedTrade).toHaveProperty('trade');
    expect(expectedTrade).toHaveProperty('expectedAmount');
  });
  ...
  it('Should return an error if no pair is available', async () => {
    ...
    await expect(async () => {
      await ref.estimateBuyTrade(ETH, DAI, '1');
    }).rejects.toThrow(UniswapishPriceError);
  });
});
