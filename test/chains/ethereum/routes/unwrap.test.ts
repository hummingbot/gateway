...
describe('Ethereum Unwrap Route', () => {
  ...
  describe('POST /chains/ethereum/unwrap', () => {
    ...
    it('should unwrap WETH successfully', async () => {
      const mockWallet = {
    ...
    it('should handle unwrapping on different networks', async () => {
      const networks = [
    ...
    it('should handle insufficient WETH balance error', async () => {
      const mockWallet = {
        address: mockWalletAddress,
        sendTransaction: jest.fn().mockRejectedValue(new Error('execution reverted: Insufficient balance')),
      };
      ...
      const data = JSON.parse(response.body);
      expect(data.message).toContain('Failed to unwrap');
    });

    it('should handle insufficient ETH for gas error', async () => {
      const mockWallet = {
        address: mockWalletAddress,
        sendTransaction: jest.fn().mockRejectedValue(new Error('insufficient funds for gas * price + value')),
      };
    ...
    it('should handle partial WETH amounts', async () => {
      const mockWallet = {
        address: mockWalletAddress,
        sendTransaction: jest.fn().mockResolvedValue({
          hash: mockTxHash,
