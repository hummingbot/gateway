...
beforeAll(async () => {
  ...
  // (src/wallet/utils.ts) — and never reads a network from the request. Holding a
  // different instance here left every `patch(eth, ...)` below applying to an object the
  // route never touched, so the "mocked" add ran the real key derivation.
...
beforeEach(() => {
  ...
  //
  // Patched on the prototype rather than on `eth`: whichever Ethereum instance the route
  ...
  // at n=131072 — roughly a second of CPU per call.
