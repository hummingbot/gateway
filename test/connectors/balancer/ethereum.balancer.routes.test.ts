...
const patchEstimateBuyTrade = () => {
  patch(balancer, "estimateBuyTrade", () => {
    return {
      expectedAmount: {
        toSignificant: () => 100,
      },
...
const patchEstimateSellTrade = () => {
  patch(balancer, "estimateSellTrade", () => {
    return {
      expectedAmount: {
        toSignificant: () => 100,
      },
    ...
    await request(app)
      ...
      .then((res: any) => {
        ...
        expect(res.body.gasCost).toEqual(
          gasCostInEthString(100, balancer.gasLimitEstimate)
        );
      });
  });
