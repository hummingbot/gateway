...
const TEST_POOL = '0x88a43bbdf9d098eec7bceda4e2494615dfd9bb9c'; // WBNB-USDC on Base
...
// Tests
describe('Pancakeswap AMM Tests (Base Network)', () => {
  beforeEach(() => {
  ...
  describe('Position Info Endpoint', () => {
    test('returns and validates position info', async () => {
      const mockResponse = {
        poolAddress: TEST_POOL,
        positionId: 'pancakeswap-v2-lp-123456',
        lpTokenAmount: 100.5,
    ...
    test('handles insufficient balance error', async () => {
      ...
      axios.post.mockRejectedValueOnce({
        response: {
          ...
          data: {
            error: 'BadRequest',
            message: 'Insufficient balance for WBNB',
            code: 400,
          },
        },
      });
