...
// Tests
describe('Pancakeswap V3 Swap Router Tests (Base Network)', () => {
  beforeEach(() => {
  ...
  describe('Execute Swap Endpoint', () => {
    test('returns successful swap execution with Pancakeswap V3 Swap Router', async () => {
    ...
    test('executes BUY swap successfully', async () => {
      ...
      const buyQuoteResponse = {
        ...
        estimatedAmountOut: 1.0, // WBNB to receive
        minAmountOut: 1.0,
      ...
      expect(response.data.amountOut).toBe(1.0); // WBNB received
      expect(response.data.baseTokenBalanceChange).toBe(1.0); // +1 WBNB
