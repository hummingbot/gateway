...
import { ElephantConfig } from './elephant/elephant.config';
...
export namespace ConnectorsRoutes {
  ...
  router.get(
    '/',
    asyncHandler(async (_req:any, res: Response<ConnectorsResponse, {}>) => {
      res.status(200).json({
        connectors: [
          {
            name: 'uniswap',
          ...
          {
            name: 'elephant',
            trading_type: ElephantConfig.config.tradingTypes,
            chain_type: ElephantConfig.config.chainType,
            available_networks: ElephantConfig.config.availableNetworks,
          },
          {
            name: 'uniswapLP',
