...
import { ElephantConfig } from './elephant.config';
...
export class Elephant implements Uniswapish {
  private static _instances: { [name: string]: Elephant };
  ...
  private constructor(network: string) {
    const config = ElephantConfig.config;
    this.harmony = Harmony.getInstance(network);
    this.chainId = this.harmony.chainId;
    this._router = '0x9B5747c6C7e29Aab9545233D6310FC6db1e11F70';
    this._ttl = config.ttl;
    this._routerAbi = routerAbi.abi;
    this._gasLimitEstimate = config.gasLimitEstimate;
  }

  public static getInstance(chain: string, network: string): Elephant {
    if (Elephant._instances === undefined) {
        Elephant._instances = {};
    }
    if (!(chain + network in Elephant._instances)) {
        Elephant._instances[chain + network] = new Elephant(network);
    }

    return Elephant._instances[chain + network];
  }
  ...
  public getAllowedSlippage(allowedSlippageStr?: string): Percent {
    ...
    const allowedSlippage = ElephantConfig.config.allowedSlippage;
   ...
   * @param elephantrouter smart contract address
  ...
  async executeTrade(
    ...
    const contract = new Contract(elephantrouter, abi, wallet);
