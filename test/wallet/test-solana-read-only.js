...
async function testReadOnlyWallet() {
  ...
  try {
    ...
    const addResponse = await axios.post(
      `${GATEWAY_URL}/wallet/add-read-only`,
      {
        chain: 'solana',
        address: TEST_WALLET,
      },
    ...
    if (solanaWallets && solanaWallets.readOnlyWalletAddresses) {
      const hasWallet =
        solanaWallets.readOnlyWalletAddresses.includes(TEST_WALLET);
      console.log(
        `✓ Read-only wallet ${hasWallet ? 'found' : 'NOT found'} in list`,
      );
      console.log('  Regular wallets:', solanaWallets.walletAddresses);
      console.log(
        '  Read-only wallets:',
        solanaWallets.readOnlyWalletAddresses,
      );
    } else {
    ...
    Object.entries(balanceResponse.data.balances).forEach(
      ([token, balance]) => {
    ...
    const allBalancesResponse = await axios.post(
      `${GATEWAY_URL}/chains/solana/balances`,
      {
        network: 'mainnet-beta',
        address: TEST_WALLET,
    ...
    const nonZeroBalances = Object.entries(
      allBalancesResponse.data.balances,
    ).filter(([_, balance]) => balance > 0);

    console.log(
      `✓ Found ${nonZeroBalances.length} tokens with non-zero balances:`,
    );
    ...
    const removeResponse = await axios.delete(
      `${GATEWAY_URL}/wallet/remove-read-only`,
      {
        data: {
          chain: 'solana',
          address: TEST_WALLET,
    ...
    const finalSolanaWallets = finalListResponse.data.find(
      (w) => w.chain === 'solana',
    );

    if (finalSolanaWallets && finalSolanaWallets.readOnlyWalletAddresses) {
      const stillExists =
        finalSolanaWallets.readOnlyWalletAddresses.includes(TEST_WALLET);
      console.log(
        `✓ Wallet ${stillExists ? 'still exists (ERROR!)' : 'successfully removed'}`,
      );
    } else {
